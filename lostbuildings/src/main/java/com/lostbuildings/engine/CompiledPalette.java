...
public class CompiledPalette {
    ...
    // Fast predictable 128-value random used by get(char) (replaces the original static fastrand128).
    private static int fastrand = 12345;

    private static int fastrand128() {
        fastrand = (fastrand * 1103515245 + 12345) & 0x7fffffff;
        return fastrand & 0x7f;
    }
        ...
        for (Palette p : palettes) {
            if (p != null) {
                for (Map.Entry<BlockState, BlockState> entry : p.getDamaged().entrySet()) {
                    damagedToBlock.put(entry.getKey(), entry.getValue());
                }
                for (Map.Entry<Character, Palette.PE> entry : p.getPalette().entrySet()) {
                    Palette.PE pe = entry.getValue();
