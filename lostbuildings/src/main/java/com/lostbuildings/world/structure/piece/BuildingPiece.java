 ...
 * down and excavate, then hand the site to {@code BuildingEngine}. Nothing about how a building is
 * built changed in wave 1 — only <em>when</em> and <em>where</em> the code runs.
 *
 * <p>What did change is that every decision the group used to make per firing (which building, how
 * many storeys, which rotation, which style, what ground level) is now decided once when the
 * structure start is assembled and carried in this piece's NBT. That is what lets a city span an
 * arbitrary number of chunks: neighbouring cells agree on a ground level and a style without ever
 * being generated in the same pass.
 */
